...
fn to_digits(line: &str) -> (u32, u32) {
    ...
    fn find(
        ...
        if let Some(ind) = line.find(st) {
            *min = Some(min.map_or((ind, dig), |prev| prev.min((ind, dig))));
        }
        if let Some(ind) = line.rfind(st) {
            *max = Some(max.map_or((ind, dig), |prev| prev.max((ind, dig))));
        }
    }
...
fn parse(input: &str) -> Vec<String> {
    input.split("\n").map(|chunk| chunk.to_owned()).collect()
}
...
fn part1(input: &[String]) -> u32 {
    input
        ...
        .map(|l| {
            let x = l
                .chars()
                .filter_map(|x| x.to_string().parse::<u32>().ok())
                .collect::<Vec<_>>();
            x[0] * 10 + x[x.len() - 1]
        })
        .sum()

...
fn parse(inp: &str) -> Vec<Inp> {
    fn parse_inner(l: &str, token: &str) -> (isize, isize) {
        let mut l = l.split(": ").nth(1).unwrap()
            .split(", ");
        let x = l.next().unwrap().split(token).nth(1).unwrap().parse::<isize>().unwrap();
        let y = l.next().unwrap().split(token).nth(1).unwrap().parse::<isize>().unwrap();
        (x, y)
    }
    inp.split("\n\n").map(|l| {
...
fn solve(inp: &Inp, offset: isize) -> isize {
    ...
    let (top, bot) = (target_y * inp.a.0 - target_x * inp.a.1, inp.b.1 * inp.a.0 - inp.b.0 * inp.a.1);
     if bot == 0 || top % bot != 0 {
        0
    } else {
